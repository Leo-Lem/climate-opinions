...
# ------------- Imports ---------------
from twikit import Client, TooManyRequests  # Importing Twitter client and error handling for rate limits
...
from random import randint  # For generating random numbers (used in mock tweets)
import random 
...
# ------------- File Names ---------------
config_filename = os.path.join(data_dir, 'configTwitter.ini')  # Config file (must exist)
cookies_filename = os.path.join(data_dir, 'cookiesTwitter.json')  # Cookies file name
log_filename = os.path.join(log_dir, f'crawlTwitter_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')  # Log file with timestamp
...
MINIMUM_TWEETS = 500  # Minimum number of tweets for production mode -> doesn't work correctly, Twitter seems to find only up to 250 entries per query.
...
def get_mock_tweets():
    ...
    return [
        MockTweet(
            user=MockUser(f'TestUser{randint(1, 100)}', randint(100, 1000), 'Mock Location'),
            ...
            place_data={'full_name': 'Mock City'} if random.choice([True, False]) else None
        )
...
async def initialize_cookies(client):
    ...
    if os.path.exists(cookies_filename):  # Check if cookies already exist
        ...
        try:
            ...
            logging.error(f'Error: {str(e)}')  # Log any error that occurs during login
            raise
...
async def get_tweets(client, query, tweets=None):
    ...
    if tweets is None:
        ...
        return await client.search_tweet(query, product='Top')  # Get tweets from real API
    else:
...
async def crawl_query(client, query, tweet_limit):
    ...
    while tweet_count < tweet_limit:  # Continue until tweet limit is reached
        try:
            ...
            await asyncio.sleep((reset_time - datetime.now()).total_seconds())  # Wait until rate limit resets
            continue
...
async def crawl_time_period(client, start_year, start_month, end_year, end_month, tweet_limit, search_query):
    ...
    while current_year < end_year or (current_year == end_year and current_month <= end_month):
        query = generate_query(current_year, str(current_month), search_query)  # Generate query for the current month

        # Define the filename for the CSV
        csv_filename = os.path.join(csv_dir, f'Twitter_{current_year}_{current_month}.csv')

        logging.info(f'Starting crawling for month: {current_year}-{current_month}')
        
        # Call the crawl_query function to get the tweet data
        try:
            tweet_data_list = await crawl_query(client, query, tweet_limit)
        except Exception as e:
            logging.error(f"Error while crawling tweets for query '{query}': {str(e)}")
            tweet_data_list = []  # Optional: Rückgabe einer leeren Liste bei Fehler
        save_to_csv(tweet_data_list, csv_filename) # Save the collected tweet data to CSV
...
async def main():
    configure_loggers()
    if TEST_MODE:
        logging.info(f'Running in test mode with API {"enabled" if USE_REAL_API_IN_TEST is True else "disabeld"}')
    ...
    if (not TEST_MODE or (TEST_MODE and USE_REAL_API_IN_TEST)):
        ...
        logging.warning('No cookies loaded because we are using mock data in test mode.')

    tweet_limit = TEST_TWEETS_PER_MONTHS if TEST_MODE else MINIMUM_TWEETS  # Set Tweet-Limit in accordance with mode.
...
# asyncio.run() to start main program
asyncio.run(main())